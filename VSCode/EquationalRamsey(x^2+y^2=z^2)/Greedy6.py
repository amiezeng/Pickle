...
def greedy_coloring(n, num_colors):
    ...
    for z in range(2, n + 1):
        ...
        for x in range(1, z):
            for y in range(x, z):
                if x**2 + y**2 == z**2:
                    ...
                    if COL.get(x) == COL.get(y) == 3:
                        color_3_allowed = False
                    if COL.get(x) == COL.get(y) == 4:
                        color_4_allowed = False
                    if COL.get(x) == COL.get(y) == 5:
                        color_5_allowed = False
                    if COL.get(x) == COL.get(y) == 6:
                        color_6_allowed = False
        ...
        if color_1_allowed:
            ...
            COL[z] = 5
        elif num_colors>=6 and color_6_allowed:
            COL[z] = 6
        else:
